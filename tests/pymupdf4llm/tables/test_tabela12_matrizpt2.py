...
def extrair_primeira_tabela_llm(pdf_path: Path, strategy="lines_strict", pagina=22):
    """
...
def extrair_primeira_tabela_pymupdf(pdf_path: Path, strategy="lines_strict", pagina=22):
    """
...
def test_primeira_tabela_com_llm(pdf_teste):
    ...
    for strategy in estrategias:
        tabela, estrutura_completa = extrair_primeira_tabela_llm(pdf_teste, strategy, pagina=22)
    ...
    if tabela is None:
        print("\npymupdf4llm não encontrou tabelas na página 22, tentando PyMuPDF diretamente...")
        for strategy in estrategias:
            tabela, estrutura_completa = extrair_primeira_tabela_pymupdf(pdf_teste, strategy, pagina=22)
            if tabela is not None:
                estrategia_usada = strategy
    ...
    esperado = {
        ...
        (2, 2): "NDEA",
        (3, 0): "3ARP320001",
        (3, 1): "Below detection  limit",
        (3, 2): "Below detection  limit",
        (4, 0): "3ARP320002",
        (4, 1): "Below detection  limit",
        (4, 2): "Below detection  limit",
        (5, 0): "3ARP320003",
        (5, 1): "Below detection  limit",
        (5, 2): "Below detection  limit",
        (6, 0): "Detection Limit",
        (6, 1): "0.01",
        (6, 2): "0.01",
        (7, 0): "Quantitation Limit",
        (7, 1): "0.03",
        (7, 2): "0.03"
    }
...
def test_matriz_ascii_comparacao_imagem(pdf_teste):
    ...
    |                    | NDMA                   | NDEA                   |
    ------------------------------------------------------------------------
    | 3ARP320001         | Below detection  limit | Below detection  limit |
    ------------------------------------------------------------------------
    | 3ARP320002         | Below detection  limit | Below detection  limit |
    ------------------------------------------------------------------------
    | 3ARP320003         | Below detection  limit | Below detection  limit |
    ------------------------------------------------------------------------
    | Detection Limit    | 0.01                   | 0.01                   |
    ------------------------------------------------------------------------
    | Quantitation Limit | 0.03                   | 0.03                   |
    ------------------------------------------------------------------------
    """
...
------------------------------------------------------------------------"""
    ...
    for strategy in estrategias:
        ...
        # Busca especificamente na página 22 (índice 21)
        idx_pagina = 22 - 1
        if idx_pagina < len(chunks):
            ch = chunks[idx_pagina]
            tabelas = ch.get("tables") or []
