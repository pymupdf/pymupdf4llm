...
def extrair_primeira_tabela_llm(pdf_path: Path, strategy="lines_strict"):
    """
    Extrai a primeira tabela usando PyMuPDF4LLM (fork local).
    ...
    print(f"\nBuscando tabelas com estratégia '{strategy}'...")
...
def extrair_primeira_tabela_pymupdf(pdf_path: Path, strategy="lines_strict"):
    """
    Extrai a primeira tabela usando PyMuPDF diretamente (fallback).
...
def test_primeira_tabela_com_llm(pdf_teste):
    ...
    for strategy in estrategias:
        tabela, estrutura_completa = extrair_primeira_tabela_llm(pdf_teste, strategy)
    ...
    if tabela is None:
        print("\npymupdf4llm não encontrou tabelas, tentando PyMuPDF diretamente...")
        for strategy in estrategias:
            tabela, estrutura_completa = extrair_primeira_tabela_pymupdf(pdf_teste, strategy)
            if tabela is not None:
                estrategia_usada = strategy
