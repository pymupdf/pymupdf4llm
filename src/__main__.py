...
def main():
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use PyMuPDF-Office for extended Office support (default: False). Full functionality requires a license key.",
    )
    parser.add_argument(
        "--office-key",
        default=None,
        help="License key for PyMuPDF-Office (default: None). May be a file path or a string.",
    )
    ...
    if args.office:
        ...
        if isinstance(args.office_key, str):
            if Path(args.office_key).is_file():
                key = Path(args.office_key).read_text().strip()
            else:
                key = args.office_key
