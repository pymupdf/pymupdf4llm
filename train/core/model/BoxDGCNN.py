...
def MLP(channels, batch_norm=True):
    ...
    for i in range(1, len(channels)):
        layer = [Linear(channels[i - 1], channels[i]), ReLU()]
        if batch_norm:
...
class BoxDGCNN(torch.nn.Module):
    def __init__(self, node_feature_dim, edge_feature_dim,
        ...
        super(BoxDGCNN, self).__init__()
        self.num_node_classes = num_node_classes
        self.num_edge_classes = num_edge_classes

        # 1. Node 특징 임베딩
        self.node_embed = torch.nn.Sequential(
            torch.nn.BatchNorm1d(node_feature_dim, affine=True),
    ...
    def forward(self, data):
        ...
        gcn_feat_1 = self.conv1(box_feat, batch)
        gcn_feat_2 = self.conv2(gcn_feat_1, batch)
        gcn_feat = self.dconv_out(torch.cat([gcn_feat_1, gcn_feat_2], dim=1))
